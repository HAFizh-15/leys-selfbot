...
client.on('credentials-updated', () => {
    ...
    fs.writeFileSync('./leys-self.json', JSON.stringify(authInfo, null, '\t'))
})

fs.existsSync('./leys-self.json') && client.loadAuthInfo('./leys-self.json')
...
_Powered by nodejs 14.x.x_`
				...
				case 'picture':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						data = await imgbb("INPUT KEY IMGBB", owgi)
				...
				*/
				...
				case 'mediafire':
					...
					reply(teks)
					client.sendMessage(from, buffer, MessageType.document, { mimetype: "application/vnd.android.package-archive" })
					break
				case 'moddroid':
